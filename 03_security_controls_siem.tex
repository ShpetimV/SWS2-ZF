...
\begin{itemize}
\item \kw{SOC (Security Operations Center)}: analysts 24/7, chase alerts.
...
\end{itemize}
Bottom = trivial for attacker to change, top = must reinvent. \textit{Ex: a \kw{hash} = high accuracy (matches exactly one file) but \hl{trivial pain} -- attacker flips 1 bit / recompiles $\to$ new hash $\to$ your block is useless $\Rightarrow$ whack-a-mole. A \kw{Tactic, Technique and Procedure (TTP)} = high pain (attacker must change how they operate) but lower accuracy (legit admins also use e.g. PowerShell $\to$ false positives).}
